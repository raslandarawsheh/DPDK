...
typedef struct __attribute__ ((packed)) pmd_flow_attr {
...
struct rxq {
	...
	/*
	 * For each VLAN ID we need separate flow steering rule
	 */
...
struct txq {
	...
	drv_exp_drain_cq_func ibv_exp_drain_cq;
	uint32_t mlx4_send_with_none;
...
mlx4_tx_burst(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	unsigned int elts_comp_req_countdown = txq->elts_comp_req_countdown;
	...
	for (i = 0; (i != max); ++i) {
		...
		/* Retrieve Memory Region key for this memory pool. */
		uint32_t lkey = txq_mp2mr(txq, buf->pool);
		...
		if (buf->ol_flags &
			...
			vendor_send_flags |= txq->mlx4_send_with_csum;
#ifdef PKT_TX_UDP_TUNNEL_PKT
		if (buf->ol_flags & PKT_TX_UDP_TUNNEL_PKT) {
			vendor_send_flags &= ~txq->mlx4_send_with_csum;
			vendor_send_flags |= txq->mlx4_send_with_csum_tunnel;
		}
#endif
		...
		/* Post packet to HW send queues */
		txq->ibv_exp_post_send_raw_one(txq->qp, addr, length, lkey,
					       vendor_send_flags);
	}

	/* Make sure HW starts working on the queued requests */
	txq->ibv_exp_flush_send(txq->qp);
...
stop:
	...
	txq->elts_comp += elts_comp;
	txq->elts_comp_req_countdown = elts_comp_req_countdown;
	  ...
	  unsigned int socket, const struct rte_eth_txconf *conf)
{
	...
	tmpl.ibv_exp_post_send_raw_one =
		...
		ibv_exp_get_provider_func(tmpl.cq->context,
					  IBV_EXP_POST_SEND_RAW_ONE_FUNC);
	tmpl.ibv_exp_post_send_raw_sgl =
		(drv_exp_post_send_raw_sgl_func)(uintptr_t)
		ibv_exp_get_provider_func(tmpl.cq->context,
					  IBV_EXP_POST_SEND_RAW_SGL_FUNC);
	tmpl.ibv_exp_flush_send =
		(drv_exp_flush_send_func)(uintptr_t)
		ibv_exp_get_provider_func(tmpl.cq->context,
					  IBV_EXP_FLUSH_SEND_FUNC);
	tmpl.ibv_exp_drain_cq =
		(drv_exp_drain_cq_func)(uintptr_t)
		ibv_exp_get_provider_func(tmpl.cq->context,
					  IBV_EXP_DRAIN_CQ_FUNC);
	if (!tmpl.ibv_exp_post_send_raw_one ||
	    !tmpl.ibv_exp_post_send_raw_sgl ||
	    !tmpl.ibv_exp_flush_send ||
	    !tmpl.ibv_exp_drain_cq) {
		ERROR("Cant find QP fast send APIs"
		      " (raw_one=%p, raw_sle=%p, flsuh_send=%p, drain_cq=%p)",
		      (void *)(uintptr_t)tmpl.ibv_exp_post_send_raw_one,
		      (void *)(uintptr_t)tmpl.ibv_exp_post_send_raw_sgl,
		      (void *)(uintptr_t)tmpl.ibv_exp_flush_send,
		      (void *)(uintptr_t)tmpl.ibv_exp_drain_cq);
		goto error;
	}
	tmpl.mlx4_send_with_none =
		ibv_exp_query_post_send_flag(tmpl.qp, 0);
	tmpl.mlx4_send_with_comp_req =
		ibv_exp_query_post_send_flag(tmpl.qp, IBV_SEND_SIGNALED);
	tmpl.mlx4_send_with_csum =
		ibv_exp_query_post_send_flag(tmpl.qp, IBV_EXP_SEND_IP_CSUM);
	tmpl.mlx4_send_with_csum_tunnel =
...
rxq_alloc_elts(struct rxq *rxq, unsigned int elts_n, struct rte_mbuf **pool)
{
	...
	for (i = 0; (i != elts_n); ++i) {
		 ...
		 * its data. */
		wr->wr_id = 0; /* wr_id not used or referred to */
		 ...
		 * from WR ID. */
		if ((void *)(sge->addr - WR_ID(wr->wr_id).offset) != buf) {
...
/**
 * Unregister all MAC addresses with specific VLAN from a RX queue.
 *
 * @param rxq
 *   Pointer to RX queue structure.
 *  @vlan_index
 ...
 */
static void
rxq_mac_addrs_vlan_del(struct rxq *rxq, unsigned int vlan_index)
{
	struct priv *priv = rxq->priv;
	unsigned int i;
	for (i = 0; (i != elemof(priv->mac)); ++i) {
		if (!BITFIELD_ISSET(rxq->mac_configured, i)) {
			continue;
		}
		rxq_del_flow(rxq, i, vlan_index);
	}
}
 ...
 *   Pointer to RX queue structure.
 * @param attr
 *   Pointer to flow attr to use
 * @param mac_index
 *   MAC address index to register.
 * @param vlan_index
 *   VLAN index. If vlan_index = -1 then the flow is without vlan
 * @return
 *   0 on success, errno value on failure.
 */
static int
rxq_add_flow(struct rxq *rxq, unsigned int mac_index, int vlan_index)
{
	...
	pmd_flow_attr flow_attr;
	/*
	 * No padding must be inserted by the compiler between attr and spec.
	 * This layout is expected by libibverbs.
	 */
	assert((uint8_t *)(&flow_attr) + sizeof(flow_attr.attr) == (uint8_t *)(&(flow_attr.spec)));
	flow_attr.attr = (struct mlx_flow_attr) {
		.type = MLX_FLOW_ATTR_NORMAL,
		.num_of_specs = 1,
		.port = priv->port,
		.flags = 0
	};
	flow_attr.spec = (struct mlx_flow_spec_eth) {
		.type = MLX_FLOW_SPEC_ETH,
		.size = sizeof(flow_attr.spec),
		.val = {
			.dst_mac = {
				(*mac)[0], (*mac)[1], (*mac)[2],
				(*mac)[3], (*mac)[4], (*mac)[5]
		...
		.mask = {
			...
			.vlan_tag = ((vlan_index != -1) ? htons(0xfff) : 0)
		}
	};
	if (vlan_index != -1) {
		flow_attr.spec.val.vlan_tag = htons(priv->vlan_filter[vlan_index].id);
	...
	/* Create related flow. */
	errno = 0;
	if ((flow = mlx_create_flow(rxq->qp, &(flow_attr.attr))) == NULL) {
...
/**
 * Register all MAC address with VLAN in a RX queue.
 *
 * @param rxq
 *   Pointer to RX queue structure.
 * @param vlan_index
 ...
 */
static int
rxq_mac_addrs_vlan_add(struct rxq *rxq, unsigned int vlan_index)
{
	struct priv *priv = rxq->priv;
	unsigned int i;
	int ret;
	...
	for (i = 0; (i != elemof(priv->mac)); ++i) {
		if (!BITFIELD_ISSET(priv->mac_configured, i))
			continue;
		ret = rxq_add_flow(rxq, i, vlan_index);
		if (!ret)
			continue;
		/* Failure, rollback. */
		while (i != 0)
...
rxq_allmulticast_enable(struct rxq *rxq)
{
	...
	if (rxq->allmulti_flow != NULL)
		return EBUSY;
	errno = 0;
	if ((flow = mlx_create_flow(rxq->qp, &attr)) == NULL) {
...
mlx4_rx_burst_sp(void *dpdk_rxq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	unsigned int elts_tail = rxq->elts_tail;
	...
	for (i = 0; (i != wcs_n); ++i) {
		...
		struct rxq_elt_sp *elt = &(*elts)[elts_tail];
		if (++elts_tail >= elts_n)
			elts_tail = 0;
		...
		assert(elts_tail < rxq->elts_n);
		assert(rxq->elts_tail < rxq->elts_n);
...
mlx4_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	for (i = 0; (i != wcs_n); ++i) {
		struct ibv_exp_wc *wc = &wcs[i];
		struct rxq_elt *elt = &(*elts)[elts_tail];
		if (++elts_tail >= elts_n)
			elts_tail = 0;
	...
	*next = NULL;
	/* Repost WRs. */
#ifdef DEBUG_RECV
	DEBUG("%p: reposting %d WRs (%p)",
	      (void *)rxq, wcs_n, (void *)head.next);
#endif
	i = ibv_post_recv(rxq->qp, head.next, &bad_wr);
	if (unlikely(i)) {
		/* Inability to repost WRs is fatal. */
...
vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id, int on)
{
	...
	if ((on) && (!priv->vlan_filter[j].enabled)) {
		/*
		 * If this is first VLAN that is set, need to remove all flows without VLAN first
		 */
		if (!vlans) {
			if (priv->rss)
				rxq_mac_addrs_vlan_del(&priv->rxq_parent, 0);
			else
				for (i = 0; (i != priv->rxqs_n); ++i) {
					if ((*priv->rxqs)[i] == NULL)
						continue;
					rxq_mac_addrs_vlan_del((*priv->rxqs)[i], 0);
				}
		}
		/*
		...
		if (priv->started) {
			if (priv->rss)
				rxq_mac_addrs_vlan_add(&priv->rxq_parent, j);
			else
				for (i = 0; (i != priv->rxqs_n); ++i) {
					if ((*priv->rxqs)[i] == NULL)
						continue;
					rxq_mac_addrs_vlan_add((*priv->rxqs)[i], j);
				}
		}
	}
		 ...
		 */
		if (priv->rss)
			rxq_mac_addrs_vlan_del(&priv->rxq_parent, j);
		else
			for (i = 0; (i != priv->rxqs_n); ++i)
				if ((*priv->rxqs)[i] != NULL)
					rxq_mac_addrs_vlan_del((*priv->rxqs)[i], j);
		priv->vlan_filter[j].enabled = 0;

		/*
		 * If this is last VLAN that was removed need to add all flows with MACs without VLAN
		 */
		if (!--vlans) {
			if (priv->rss)
				rxq_mac_addrs_vlan_add(&priv->rxq_parent, -1);
			else
				for (i = 0; (i != priv->rxqs_n); ++i)
					if ((*priv->rxqs)[i] != NULL)
						rxq_mac_addrs_vlan_add((*priv->rxqs)[i], -1);
		}
	}
	return 0;
}
...
/**
 * Derive MAC address from port interface.
 *
 * @param[out] mac
...
static int
get_mac_from_interface(struct priv *priv, uint8_t (*mac)[ETHER_ADDR_LEN])
{
        int ret;
        struct ifreq request;

        ret = priv_ifreq(priv, SIOCGIFHWADDR, &request);
        if (ret < 0) {
                ERROR("Failed to get MAC address, error: %s\n", strerror(errno));
                return ret;
        }
...
mlx4_pci_devinit(struct rte_pci_driver *pci_drv, struct rte_pci_device *pci_dev)
{
	...
	for (i = 0; i < device_attr.phys_port_cnt; i++) {
		...
		get_mac_from_interface(priv,&mac.addr_bytes);
