...
pmd = '/download/mlx4_pmd_beta_v2.8.1/librte_pmd_mlx4/librte_pmd_mlx4.so'
mtuset_pmd = '/download/mlx4_pmd_beta_v2.8.1.mtuset/librte_pmd_mlx4/librte_pmd_mlx4.so'
checksum_pmd = '/download/mlx4_pmd_beta_v2.8.1.checksum/librte_pmd_mlx4/librte_pmd_mlx4.so'
mtuset_and_checksum_pmd = '/download/mlx4_pmd_beta_v2.8.1.mtuset.checksum/librte_pmd_mlx4/librte_pmd_mlx4.so'
pmd_debug = '/download/mlx4_pmd_beta_v2.8.1.debug/librte_pmd_mlx4/librte_pmd_mlx4.so'
mtuset_pmd_debug = '/download/mlx4_pmd_beta_v2.8.1.mtuset.debug/librte_pmd_mlx4/librte_pmd_mlx4.so'
checksum_pmd_debug = '/download/mlx4_pmd_beta_v2.8.1.checksum.debug/librte_pmd_mlx4/librte_pmd_mlx4.so'
mtuset_and_checksum_pmd_debug = '/download/mlx4_pmd_beta_v2.8.1.mtuset.checksum.debug/librte_pmd_mlx4/librte_pmd_mlx4.so'
recvinline_pmd = '/download/mlx4_pmd_beta_v2.8.1.recvinline/librte_pmd_mlx4/librte_pmd_mlx4.so'
recvinline_pmd_debug = '/download/mlx4_pmd_beta_v2.8.1.recvinline.debug/librte_pmd_mlx4/librte_pmd_mlx4.so'
...
def parse(tuple):
    ...
    global accepted_sock, connect_sock, ip_hw, kvm, generator_device, generator_port, vmware, master_rc
...
#       if kvm or vmware:
#           os.system(pmd.split('librte_pmd_mlx4/librte_pmd_mlx4.so')[0] + "/configure_mlx4_pmd.sh -s kvm")
#       else:
#	    if dmfs == 'A0':
	   ...
	   '''
        ...
        if vlanFiltering == '2':
            ...
            if vlan4_data:
                ...
                cur_tx = int(cur_tx_p0) + int(cur_tx_p1)
                if (cur_rx - old_rx) != 1:
                    print "Testpmd did not received packet for vlan 4 after adding two vlans"
                    failPrint()
                    sys.exit(1)
            else:
                print "Sync : Recv error"
            ...
            if vlan4_data:
                ...
                cur_tx = int(cur_tx_p0) + int(cur_tx_p1)
                if (cur_rx - old_rx) != 1:
                    print "Testpmd did not received packet for vlan 5 after adding two vlans"
                    failPrint()
                    sys.exit(1)
            else:
                print "Sync : Recv error"
            ...
            if vlan4_data:
                ...
                cur_tx = int(cur_tx_p0) + int(cur_tx_p1)
                if (cur_rx - old_rx) != 1:
                    print "Testpmd did not receive packet for vlan 4 after removing All vlans"
                    failPrint()
                    sys.exit(1)
            else:
                print "Sync : Recv error"
            ...
            if vlan4_data:
                ...
                cur_tx = int(cur_tx_p0) + int(cur_tx_p1)
                if (cur_rx - old_rx) != 1:
                    print "Testpmd did not receive packet for vlan 5 after removing All vlans"
                    failPrint()
                    sys.exit(1)
            else:
                print "Sync : Recv error"
...
#bbb
        ...
        #waiting for the first order to send packet
            ...
            if vlan_data1:
                ...
                os.popen(cmd)
            connect_sock.send("VLAN4")
            print "waiting for vlan5 order after the server has removed all vlans "
            vlan_data1 = connect_sock.recv(BUF_LEN)
            if vlan_data1:
                cmd = "UTscapy -t vlanFilter5.txt -f html -o demo_campaign.html -F"
                os.popen(cmd)
