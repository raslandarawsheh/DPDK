...
def bidirectionalTest(is_server,child,destMac,ip,cip_port1,cip_port2,NIC,mtuSet,invalidMac):
    ...
    if is_server :
        ...
        for i in range(1,5) : 
            ...
            cur_rx0 = int(cur_rx_p0)
            cur_tx0 = int(cur_tx_p0)
            cur_rx1 = int(cur_rx_p1)
            cur_tx1 = int(cur_tx_p1)
            if ((cur_rx0 - old_rx0) == 0) or ((cur_rx1 - old_rx1) == 0):
                print "Testpmd did not received packet for Bidirectional port 0 :    fail"
                if mtuSet == None :
                    pass
                else :
            ...
            elif ((cur_tx0 - old_tx0) == 0) or ((cur_tx1 - old_tx1) == 0):
                print "Testpmd did not received packet for Bidirectional  port 1 :    fail"
                if mtuSet == None :
                    pass
                else :
                    child.sendline("port config mtu 0 1500")
        ...
        if mtuSet == None:
            clientCommandLine = "raw_ethernet_bw --client -D 5 --CPU-freq -i " + generator_port + " -d " + generator_device + " -l 8 "
        else:
            clientCommandLine = "raw_ethernet_bw --client -D 5 --CPU-freq -i " + generator_port + " -d " + generator_device + " -l 8  --mtu " + mtuSet
            
        if NIC == "cx3" :
            if int(generator_port) == 1 :
                clientCommandLine1 = "raw_ethernet_bw --client -D 5 --CPU-freq -i 2  -d " + generator_device + " -l 8 "
            else :
                clientCommandLine1 = "raw_ethernet_bw --client -D 5 --CPU-freq -i 1  -d " + generator_device + " -l 8 "
        else :
            clientCommandLine1 = "raw_ethernet_bw --client -D 5  --CPU-freq -i 1  -d mlx5_1  -l 8 "
            
        if mtuSet == None:
            pass 
        else:
            clientCommandLine1 += " --mtu " + mtuSet
        
        clientCommandLine+=" --dest_mac=" + destMac +" & "
        clientCommandLine1 += " --dest_mac=" + destMac
        print "cmd : ", clientCommandLine 
        print "cmd1: ", clientCommandLine1
        for i in range(1,5):
            ...
            process  = subprocess.Popen(clientCommandLine1, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            process_out = process.stderr.read()
            connect_sock.send("sync1")
            data = connect_sock.recv(5)
        cmd = "ifconfig " + getNameOfIp(cip_port1) + " mtu " + str(mtuSet) + " up"
        cmd1 = "ifconfig " + getNameOfIp(cip_port2) + " mtu " + str(mtuSet) + " up"
        print cmd
        os.system(cmd)
        print cmd1
        os.system(cmd1)
...
def generateTraficForRSS3Qs(srcMac,destMac,ip,cip_port1,traffic,mtuSet,invalidMac):
    ip6 = False
    if mtuSet == None:
    ...
    if traffic != None :
        if traffic == "TCP_IPV4" :
            cmd1 = clientCommandLine + " --dest_mac=e4:1d:2d:e8:35:bc  --dest_ip "+ ip + " --source_mac=fa:fa:fa:fa:fa:f9 --source_port 5362 --dest_port 6325 --tcp"
            cmd2 = clientCommandLine + " --dest_mac=a7:fa:a4:3c:13:dd  --dest_ip "+ ip + " --source_ip 255.255.255.255  --source_port 0101 --dest_port 1010 --tcp"
            cmd3 = clientCommandLine + " --dest_mac=a7:fa:a4:3c:13:dd  --dest_ip "+ ip + " --source_ip " + cip_port1 + " --source_port 5362 --dest_port 6325 --tcp"

        elif traffic =="UDP_IPV4" :
            cmd1 = clientCommandLine + " --dest_mac=e4:1d:2d:e8:35:bc  --dest_ip "+ ip + " --source_mac=fa:fa:fa:fa:fa:f9 --source_port 5362 --dest_port 6325 "
            cmd2 = clientCommandLine + " --dest_mac=a7:fa:a4:3c:13:dd  --dest_ip "+ ip + " --source_ip 255.255.255.255  --source_port 0101 --dest_port 1010 "
            cmd3 = clientCommandLine + " --dest_mac=a7:fa:a4:3c:13:dd  --dest_ip "+ ip + " --source_ip " + cip_port1 + " --source_port 5362 --dest_port 6325 "

        elif traffic == "IPV4" :
            cmd1 = clientCommandLine + " --dest_mac=e4:1d:2d:e8:35:bc  --dest_ip "+ ip + " --source_mac=fa:fa:fa:fa:fa:f9"
            cmd2 = clientCommandLine + " --dest_mac=fa:fa:fa:fa:fa:fa --source_ip 255.255.255.255"
            cmd3 = clientCommandLine + " --port 9999  --dest_mac=e4:1d:2d:e8:35:bc  --source_ip 255.255.255.255  --dest_ip "+ ip

        elif traffic == "TCP_IPV6":
            cmd1 = clientCommandLine + ""
            cmd2 = clientCommandLine + ""
            cmd3 = clientCommandLine + ""
        ...
        if ip6 :
            ...
            cmd2 = "UTscapy -t ip6_2.txt -f html -o demo_campaign.html -F"
            cmd3 = "UTscapy -t ip6_3.txt -f html -o demo_campaign.html -F"
...
def generateTraficForRSS4Qs(srcMac,destMac,ip,cip_port1,traffic,mtuSet,invalidMac):
    ip6 = False
    if mtuSet == None:
        clientCommandLine = "raw_ethernet_bw --client --CPU-freq -i " + generator_port + " -d " + generator_device + " -l 8 "
    else:
    ...
    if traffic != None :
        if traffic == "TCP_IPV4" :
            ...
            cmd1= clientCommandLine + "  --port 9999 --source_port 20911 --dest_port 20910 --dest_mac="+ destMac + " --dest_ip 14.4.3.7 --\
source_ip 13.4.3.8 --source_mac e4:1d:2d:5c:f0:cd --tcp"
            cmd2=clientCommandLine + "  --source_mac=fa:fa:fa:fa:fa:fa  --port 9999 --source_port 2302 --dest_port 2910 --dest_mac="+ destMac + "  --dest_ip 14.4.3.7 --tcp"
            cmd3=clientCommandLine + "  --port 9999 --source_port 20911 --dest_port 20910 --dest_mac="+ destMac + " --dest_ip 14.4.3.3 --s\
ource_ip 13.4.3.8 --source_mac e4:1d:2d:5c:f0:cd --tcp"
            cmd4=clientCommandLine + "  --source_mac=fa:fa:fa:fa:fa:fa  --port 9999 --source_port 2302 --dest_port 2910 --dest_mac="+ destMac + "  --dest_ip 14.4.3.3 --tcp"
            ...
            cmd1= clientCommandLine + "  --port 9999 --source_port 20911 --dest_port 20910 --dest_mac="+ destMac + " --dest_ip 14.4.3.7 --source_ip 13.4.3.8 --source_mac e4:1d:2d:5c:f0:cd"
            cmd2=clientCommandLine + "  --source_mac=fa:fa:fa:fa:fa:fa  --port 9999 --source_port 2302 --dest_port 2910 --dest_mac="+ destMac + " --dest_ip 14.4.3.7"
            cmd3=clientCommandLine + " --port 9999 --source_port 20911 --dest_port 20910 --dest_mac="+ destMac + " --dest_ip 14.4.3.3 --source_ip 13.4.3.8 --source_mac e4:1d:2d:5c:f0:cd"
            cmd4=clientCommandLine + "  --source_mac=fa:fa:fa:fa:fa:fa  --port 9999 --source_port 2302 --dest_port 2910 --dest_mac="+ destMac + " --dest_ip 14.4.3.3"
            ...
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            #Thierd  file to send data
            scapy_file = open('ip6_3.txt','w')
            scapy_file.write("% IPv6 Tests\n")
            scapy_file.write("+ Informations\n")
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='2001:db8::1', src='fe80::f452:1400:161:a012')/TCP(sport=1337,dport=4789)/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            #forth  file to send data
            scapy_file = open('ip6_4.txt','w')
            scapy_file.write("% IPv6 Tests\n")
            scapy_file.write("+ Informations\n")
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='fe80::f652:14ff:fe2c:5d21', src='fe80::f452:1400:161:a012')/TCP(sport=1337,dport=4789)/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            ...
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='2001:db8::1', src='fe80::f452:1400:161:a012')/UDP(sport=1337,dport=4789)/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            #forth  file to send data
            scapy_file = open('ip6_4.txt','w')
            ...
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='fe80::f652:14ff:fe2c:5d21', src='fe80::f452:1400:161:a012')/UDP(sport=1337,dport=4789)/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            ...
            cmd1 =clientCommandLine + "  --dest_mac="+ destMac + " --dest_ip 15.4.3.1"
            cmd2 =clientCommandLine + "  --dest_mac="+ destMac + " --dest_ip 15.4.3.3"
            cmd3 =clientCommandLine + "  --dest_mac="+ destMac + " --dest_ip 15.4.3.4"
            cmd4 =clientCommandLine + "  --dest_mac="+ destMac + " --dest_ip 15.4.3.6"
            ...
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='2001:db8::1', src='fe80::f452:1400:161:a012')/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
            #forth  file to send data
            scapy_file = open('ip6_4.txt','w')
            ...
            scapy_file.write("\n= Send Packet via Scapy\n")
            scapy_file.write("data= 'Data to be sent via Scapy'\n")
            scapy_file.write("p =Ether(src='fa:fa:fa:fa:fa:fa', dst='%s')/IPv6(dst='fe80::f652:14ff:fe2c:5d21', src='fe80::f452:1400:161:a012')/Raw(load=data) \n" %(destMac))
            scapy_file.write("sendp(p, iface='%s')\n" %(getNameOfIp(cip_port1)))
            scapy_file.close()
        ...
        if ip6 :
            ...
            cmd2 = "UTscapy -t ip6_2.txt -f html -o demo_campaign.html -F"
            cmd3 = "UTscapy -t ip6_3.txt -f html -o demo_campaign.html -F"
            cmd4 = "UTscapy -t ip6_4.txt -f html -o demo_campaign.html -F"
        print "cmd1",cmd1
        print "cmd2",cmd2
        print "cmd3",cmd3
        print "cmd4",cmd4
...
def usage():
    ...
    print Bcolors.HELP +"\tbidirectional  \t bidirectional tests "
...
def parse(tuple):
    ...
    global accepted_sock, connect_sock, ip_hw, kvm, generator_device, generator_port, vmware, master_rc, testpmd, offloaded, traffic, NIC, bidirectional
...
#               serverCommandLine = "/download/dpdk_master_dynamic.default"
        if NIC == "cx3" :
            ...
            serverCommandLine = "/download/MLNX_DPDK-2.1-1.0"
        else :
    ...
    if is_s:
        ...
        if not (kvm or vmware):
            if NIC == "cx3" :
                ...
                pci1= getWhiteListed(device)
                pci2 = pci1.split('.')[0] + ".1"
                serverCommandLine += " -w "+ pci1 + "  -w " + pci2 +  " "            
...
################################Check Sum####################################
	if checksum != None:
	    child.sendline("set fwd csum")
            if offloaded :
                if checksum_flags[0] == True:
                    child.sendline("csum set ip hw 0")
                    child.sendline("csum set ip hw 1")
                if checksum_flags[1] == True:
                    child.sendline("csum set udp hw 0")
                    child.sendline("csum set udp hw 1")
                if checksum_flags[2] == True:
                    child.sendline("csum set tcp hw 0")
                    child.sendline("csum set tcp hw 1")
                if checksum_flags[3] == True:
                    child.sendline("csum set vxlan hw 0")
                    child.sendline("csum set vxlan hw 1")
                if checksum_flags[4] == True:
                    child.sendline("csum set vxlan hw 0")
                    child.sendline("csum set vxlan hw 1")
		...
		print "cur_rx_p0 ", cur_rx_p0, "cur_rx_p1 ", cur_rx_p1, "cur_tx_p0 ", "cur_tx_p1 ", cur_tx_p1
            ...
            accepted_sock.send("Sync3")
#bbb
	if multicast and vlanFiltering == None and mtuSetMultipleTimes ==False and bidirectional == False:
            old_rx = 0
            old_tx = 0
	    child.sendline("set promisc all off")
	    child.sendline("set allmulti all on")
        ...
        if bidirectional :
            ...
            if mtuSet ==None :    
                bidirectionalTest(True,child,destMac,ip,cip_port1,cip_port2,NIC,mtuSet,invalidMac)
            else :
                child.sendline("port config mtu 0  " + str(mtuSet))
                child.sendline("port config mtu 1  " + str(mtuSet))
                time.sleep(1)
...
#dpdk1.6                RxPacketsRate = outputLineToPrint[w].split("RX-packets:")[1].split("RX-errors:")[0].strip()
        ...
        checkOutputFile(setPromisc,invalidMac,RxPacketsRate,TxPacketsRate,add_mac,passCount,rxDataCounter,txDataCounter,rssQueuesFlage,vlanFiltering,vxlan,all_tx_packets,all_rx_packets,checksum,Bad_ipcsum,Bad_l4csum, checksum_flags,vlan4_packet,vlan5_packet,rxq,txq)
	...
	print "Doneeeeeeeeeeeeeeeeeeee"
        if mtuSetMultipleTimes:
            ...
            for i in range(len(mtu_sizes)):
                ...
                process_out = process.stdout.read()
#bbb
	elif multicast and vlanFiltering == None and mtuSetMultipleTimes == False and bidirectional == False:
	    connect_sock.send("Sync3") # send the data
	    time.sleep(3)
	    data = connect_sock.recv(BUF_LEN)
            if mtuSet == None:
	    ...
	    time.sleep(3)
            ...
            print traffic
            if traffic != None :
                if int(rxq) == 3 and int(txq) == 3  and NIC == "cx4" : 
                    generateTraficForRSS3Qs(srcMac,destMac,ip,cip_port1,traffic,mtuSet,invalidMac)
                else :
                    generateTraficForRSS4Qs(srcMac,destMac,ip,cip_port1,traffic,mtuSet,invalidMac)                
            else :
	    ...
	    if vlan_data2:
            ...
            if vlanFiltering == '1':
                ...
                print "waiting for the first order to send packet"
                vlan_data1 = connect_sock.recv(BUF_LEN)
                if vlan_data1:
                    cmd = "UTscapy -t vlanFilter4.txt -f html -o demo_campaign.html -F"
                    print cmd
		...
		if checksum_result == 1:
            ...
            if mtuSet == None :
                ...
                os.system(cmd1)
                time.sleep(4)
            bidirectionalTest(False,None,destMac,ip,cip_port1,cip_port2,NIC,mtuSet,invalidMac)
...
def checkOutputFile(setPromisc,invalidMac,RxPacketsRate,TxPacketsRate,add_mac,passCount,rxDataCounter,txDataCounter,rssQueuesFlage,vlanFiltering,vxlan,all_tx_packets,all_rx_packets,checksum,Bad_ipcsum,Bad_l4csum, checksum_flags,vlan4_packet,vlan5_packet,rxq,txq):
    TxOutput = []
    RxOutput = []
...
#            sys.exit(1)
    ...
    if rssQueuesFlage and vxlan == None:
        ...
        if int(passCount) > 1 and (int(all_rx_packets) != 0 ) and (int(all_tx_packets) != 0):
            if int(rxq) > 2 and int(txq) > 2 :
                if int(passCount) > 3 :
                    passPrint()
                    sys.exit(0)
                else :
                    if (int(rxq) == 3 or int(rxq) == 5) and int(passCount) == 3  :
                        passPrint()
                        sys.exit(0)
                    else :
                        failPrint()
